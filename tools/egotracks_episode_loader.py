...
def build_episode_annotations(
    ...
    source sequence:
        - a visible window of length `visible_frames`
        - a gap of consecutive invisible frames that follows, provided the gap
          is at least `missing_frames` long
        - a reappearance window of length `reappearance_frames` with visible
          frames immediately after the gap

    The loader scans every possible visible window of the requested size inside
    each run of consecutive visible frames. This makes it possible to generate
    multiple episodes from a single long visible run whenever each window is
    followed by a valid gap and reappearance window.
    """
    ...
    for cursor in range(n):
        ...
        for visible_end in range(run_end, run_start + visible_frames - 2, -1):
            visible_start = visible_end - visible_frames + 1
            gap_start = visible_end + 1
            ...
            reappear_run_end = reappear_start
            while reappear_run_end + 1 < n and visibility[reappear_run_end + 1]:
                reappear_run_end += 1
            reappear_run_len = reappear_run_end - reappear_start + 1

            if reappear_run_len < reappearance_frames:
                continue
            ...
            episode_info = {
                ...
                "segments": {
                    "visible": {
                        "source_line_range_0based": [visible_start, visible_end],
                        "source_frame_indices": frame_indices[visible_start : visible_end + 1],
                    },
                    "missing": {
