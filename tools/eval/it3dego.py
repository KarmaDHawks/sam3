...
class ExperimentIT3DEgo(object):
    ...
    def __init__(self, root_dir, result_dir='./results', report_dir='./reports'):
        super(ExperimentIT3DEgo, self).__init__()
        self.root_dir = root_dir
        self.dataset = IT3DEgo(root_dir)
        self.result_dir = os.path.join(result_dir, 'IT3DEgo')
        self.report_dir = os.path.join(report_dir, 'IT3DEgo')
    ...
    def run_sequence(self, idx, tracker, visualize=False, save_video=False, overwrite_result=True):
        ...
        assert len(boxes) == len(img_files)
    ...
    def report_ope(self, tracker_names, realtime=False):
        ...
        for name in tracker_names:
            ...
            for s, (_, anno, vis) in enumerate(self.dataset):
                seq_name = self.dataset.seq_names[s]
                ...
                print(seq_name, len(boxes), len(anno), vis.sum())
                assert len(boxes) == len(anno)
