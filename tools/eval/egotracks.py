...
from ..datasets import EgoTracks
from ..datasets.egotracks import EgoTracks_hic
from ..utils.metrics import rect_iou, center_error, normalized_center_error
from ..utils.viz import show_frame
from ..utils.ioutils import compress_file
sys.path.append('/home/zaira/Projects/sam2/hic-hois')
from hic_detector import HIC_Detector


class ExperimentEgoTracks(object):
    ...
    def __init__(self, root_dir, result_dir='./results', report_dir='./reports'):
        super(ExperimentEgoTracks, self).__init__()
        self.root_dir = root_dir
        self.dataset = EgoTracks(root_dir)
        self.result_dir = os.path.join(result_dir, 'EgoTracks')
        self.report_dir = os.path.join(report_dir, 'EgoTracks')
    ...
    def run_ope(self, tracker, visualize=False):
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            record_file = os.path.join(
                self.result_dir, tracker.name, 'hoi', '%s.txt' % seq_name)
            ...
            boxes, times = tracker.track(
                img_files, anno[0, :], anno=anno, visualize=visualize, seq_name=seq_name)
    ...
    def run_sequence(self, idx, tracker, visualize=False, save_video=False, overwrite_result=True):
        output =  self.dataset[idx]
 
        img_files, anno = output
        ...
        # tracking loop
        boxes, times = tracker.track(
            img_files, anno[0, :], anno=anno, visualize=visualize, dataset_name=self.dataset.name, seq_name=seq_name)
        assert len(boxes) == len(anno)

        # record results
        self._record(record_file, boxes, times)

    def run_sequential(self, tracker, visualize=False, save_video=False, overwrite_result=True):
        ...
        for s in range(len(self.dataset)):
            self.run_sequence(s, tracker, visualize=visualize)

    def run_parallel(self, tracker, visualize=False, save_video=False, overwrite_result=True, threads=1):

        print('Running tracker %s on EgoExo4D...' % tracker.name)

        multiprocessing.set_start_method('spawn', force=True)

        param_list = [(seq_idx, tracker) for seq_idx in range(len(self.dataset))]

        with multiprocessing.Pool(processes=threads) as pool:
            pool.starmap(self.run_sequence, param_list)

    def report(self, tracker_names, protocol='hoi'):
        if protocol == 'ope':
        ...
        for name in tracker_names:
            ...
            for s, (_, anno) in enumerate(self.dataset):
                ...
                if not os.path.exists(record_file):
                    print(f"⚠️  File not found: {record_file}")
                    continue  
                boxes = np.loadtxt(record_file, delimiter=',')
                #remove frames with no visible objects
                #anno = list(anno)
                #for frame_idx in reversed(frames_idx_no_obj):
                    #anno.pop(frame_idx)
                #anno = np.array(anno)
                if anno is None:
                    continue
...
"""
class ExperimentEgoTracks_hic(object):

   
    
    def __init__(self, root_dir, result_dir='./results', report_dir='./reports'):
        super(ExperimentEgoTracks_hic, self).__init__()
        self.root_dir = root_dir
        self.dataset = EgoTracks(root_dir)
    ...
    def run_ope(self, tracker, visualize=False):
        ...
        # loop over the complete dataset
        #for s, (img_files, anno) in enumerate(self.dataset):
        for s in enumerate(self.dataset):
            seq_name = self.dataset.seq_names[s]
            obj_state_detector = HIC_Detector('EgoTracks', seq_name)
            img_files = obj_state_detector.image_list
            anno = obj_state_detector.annotations
            print('--Sequence %d/%d: %s' % (s + 1, len(self.dataset), seq_name))
        ...
        for name in tracker_names:
            ...
            #for s in np.arange(seq_num):
                ...
                if not os.path.exists(record_file):
                    continue
                #anno= []
                #for frame_idx in np.arange(len(img_files)):
                 #   new_elm_anno= obj_state_detector.get_detections(frame_idx,boxes[frame_idx])[1]
                  #  if new_elm_anno is None:
                   #     new_elm_anno =np.array([-1, -1, -1, -1])
                    #anno.append(new_elm_anno)
                #anno = np.array(anno)
                boxes = np.loadtxt(record_file, delimiter=',')
                boxes = boxes[~np.isnan(anno[:, 0])]
                anno = anno[~np.isnan(anno[:, 0])]
