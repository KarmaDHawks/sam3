...
class ExperimentTREK150(object):
    ...
    def __init__(self, root_dir, result_dir='./results', report_dir='./reports'):
        ...
        self.dataset = TREK150(root_dir, download=False)
    ...
    def run_ope(self, tracker, visualize=False):
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            boxes, times = tracker.track(
                img_files, anno[0, :], visualize=visualize, seq_name=seq_name)
    ...
    def run_sequence(self, idx, tracker, visualize=False, save_video=False, overwrite_result=True):
        ...
        # tracking loop
        boxes, times = tracker.track(
            img_files, anno[0, :], visualize=visualize, dataset_name=self.dataset.name, seq_name=seq_name)
        assert len(boxes) == len(anno)
    ...
    def report_hoi(self, tracker_names):
        ...
        for name in tracker_names:
            ...
            for s, (_, anno) in enumerate(self.dataset):
                seq_name = self.dataset.seq_names[s]
                if seq_name == 'P03-P03_02-56': #they are not in the folder
                    continue
                if seq_name == 'P03-P03_04-57':
                    continue
                if seq_name == 'P05-P05_03-11':
                    continue
                if seq_name == 'P05-P05_03-21':
                    continue
                if seq_name == 'P07-P07_11-74':
                    continue
                if seq_name == 'P12-P12_02-41':
                    continue
                if seq_name == 'P12-P12_04-708':
                    continue
                if seq_name == 'P13-P13_04-10':
                    continue
                if seq_name == 'P15-P15_09-128':
                    continue
                if seq_name == 'P21-P21_01-101':
                    continue
                anchors = np.loadtxt(os.path.join(self.root_dir, seq_name, 'anchors_hoi.txt'), delimiter=',')
                if len(anchors.shape) == 1:
                    anchors = np.array([anchors])
                ...
                for i in range(anchors.shape[0]):
                    ...
                    record_file = os.path.join(
                            self.result_dir, name, 'hoi', f'{seq_name}.txt')
