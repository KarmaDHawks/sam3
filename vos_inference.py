...
def vos_inference(
    ...
    # add those input masks to SAM 3 inference state before propagation
    object_ids_set = None
    for input_frame_idx in input_frame_inds:
        ...
        for object_id, object_mask in per_obj_input_mask.items():
            ...
            # Convert numpy mask to torch tensor and add to inference state
            mask_tensor = torch.from_numpy(object_mask.astype(np.float32))
    ...
    # SAM 3 returns 5 values: frame_idx, obj_ids, low_res_masks, video_res_masks, obj_scores
    os.makedirs(os.path.join(output_mask_dir, video_name), exist_ok=True)
    output_palette = input_palette or DAVIS_PALETTE
    video_segments = {}  # video_segments contains the per-frame segmentation results
    
    print(f"Running propagation on {video_name}...")
    for frame_idx, obj_ids, low_res_masks, video_res_masks, obj_scores in predictor.propagate_in_video(
        ...
        # SAM 3 returns masks already at video resolution, no need for thresholding
        # video_res_masks already contains the final masks for all objects
        per_obj_output_mask = {
    ...
    print(f"Saving output masks for {video_name}...")
...
def main():
    ...
    # Aggiungi queste righe prima di caricare il modello
    #from huggingface_hub import login
    
    # Opzione A: Usa il token già salvato localmente
    #login()
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="threshold for the output mask (default: 0.0)",
    )
    parser.add_argument(
        "--use_all_masks",
        action="store_true",
        help="whether to use all available PNG files in input_mask_dir "
        "(default without this flag: just the first PNG file as input to the SAM 3 model; "
    ...
    if args.use_all_masks:
        print("using all available masks in input_mask_dir as input to the SAM 3 model")
    else:
        print(
